...
import { X, CloudSun, Wind, Droplets, Calendar, MapPin, ExternalLink, RefreshCw, Sun } from 'lucide-react';
...
export default function WeatherModal({ isOpen, onClose, currentWeather }) {
  const [activeTab, setActiveTab] = useState('forecast'); // 'forecast' | 'radar'
  ...
  useEffect(() => {
    ...
    async function fetchDaily() {
      ...
      try {
        const url =
          'https://api.open-meteo.com/v1/forecast?latitude=12.9276&longitude=100.8771&daily=weather_code,temperature_2m_max,temperature_2m_min,precipitation_probability_max,wind_speed_10m_max&timezone=Asia%2FBangkok';
        const res = await fetch(url);
        if (!res.ok) throw new Error('Failed to fetch weather forecast');
        const json = await res.json();
        if (isMounted && json.daily) {
          const days = [];
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-sm font-bold text-white flex items-center gap-2">
                <span>Pattaya 7-Day Forecast & Live Wind Radar</span>
              ...
              <p className="text-[11px] text-slate-400 font-mono">
                Real-time Gulf of Thailand meteorological telemetry & ECMWF/GFS predictive model
              </p>
            </div>
          </div>
          <button
            ...
            <div className="space-y-4">
              {loading && (
                <div className="flex items-center justify-center py-12 text-slate-400 gap-2">
                  ...
                  <span>Loading Gulf of Thailand 7-day forecast...</span>
                </div>
              )}
              ...
              {dailyForecast && (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-2.5">
                  {dailyForecast.map((day, idx) => (
                    ...
                    >
                      <span className={`text-[10px] font-bold uppercase ${idx === 0 ? 'text-amber-400' : 'text-slate-300'}`}>
              ...
              <div className="p-3 rounded-xl bg-canvas/70 border border-borderDark text-[11px] text-slate-300 space-y-1.5">
                ...
                <p className="text-slate-400 leading-relaxed">
                  Pattaya features a tropical wet and dry climate with three main seasons: Cool & Dry (November to February), Hot (March to May), and Rainy/Monsoon (June to October). Coastal sea breezes generally provide refreshing evening airflow along Beach Road and Jomtien.
                </p>
              </div>
            </div>
          )}
